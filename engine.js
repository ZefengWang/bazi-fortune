...
/* =====================================================================
 * ★ 文件结构索引（便于快速定位；行号会随编辑漂移，以「分节横幅」为准）
 * ---------------------------------------------------------------------
 *  0. 通用算子            第15行   pymod/norm360 等数学辅助
 *  1. 符号集与属性矩阵     第21行   TIANGAN/DIZHI/WU_XING/SHISHEN_MAP/藏干
 *  2. 历法与天文学         第77行   儒略日/节气(逼近)/均时差/真太阳时
 *  3. 时区                第208行  IANA 时区偏移 / 本地→UTC
 *  4. 格局详解数据         第275行  PATTERN_DETAILS/神煞详解字典
 *  5. 十神                第443行  十神关系 / 主藏干提取
 *  6. 核心排盘            第479行  execute_global_fortune_engine 排四柱
 *  7. 解译报告            第577行  藏干/身强弱/喜用神/十神/大运流年解读
 *  8. 大运流年            第598行  大运排布 / 流年干支
 *  9. 婚姻事业&喜用神精判  第695行  computeMarriageAndCareer/喜用神方向
 * 10. 合婚                第1054行  computeHehun+术语库（五合/六合/三合/相刑/鸳鸯合）
 * 11. 事业合盘·合伙       第1388行  computeHezuo（六维）
 * 导出                    第1576行  浏览器 window.BaziEngine / Node module.exports
 * =====================================================================
 */
...
/* =====================================================================
 * 10. 八字合婚（学习演示，仅供娱乐与研究，不构成任何婚恋建议）
 * ---------------------------------------------------------------------
 * 三维「正格」综合评分（满分 100，区间 [-100,+100]），凶一律负分、关于 0 对称：
 *   ① 年支·生肖正格（20分）六合20/三合10/平和0/害-10/刑-15/冲-20
 *   ② 日干·日主正格（40分）五合40/生20/比0/克-20/天干冲-40
 *   ③ 日支·婚姻宫正格(40分) 六合40/三合20/平和0/害-20/刑-30/冲-40
 * 另附（不计入总分，仅作参考解读）：④五行互补 ⑤喜用神互补 ⑥格局阴阳
 * 档位（关于 0 对称）：≥85 上等 / ≥55 中上 / ≥25 偏上 / -25~25 中等 / -55~-25 偏下 / -85~-55 下等 / <-85 极冲
 * 触发徽章「天地鸳鸯合」= 日干五合 且 日支六合（干合支合）
...
function computeHehun(resA, repA, resB, repB) {
  ...
  else if (score >= 55) { level = "中上等婚"; verdict = "良配，契合度较高，只要经营得当，感情生活可和谐美满。"; }
  else if (score >= 25) { level = "中等偏上婚"; verdict = "缘分不错，相合多于相冲，稍加磨合即可长久。"; }
  else if (score > -25) { level = "中等婚"; verdict = "缘分一般，有好有坏、吉凶参半，重在磨合与经营。"; }
  else if (score > -55) { level = "中等偏下婚"; verdict = "契合度偏低，冲克渐显，需双方多付出耐心与理解去经营。"; }
